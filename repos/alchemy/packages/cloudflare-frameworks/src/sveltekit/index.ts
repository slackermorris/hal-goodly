/**
 * `@alchemy.run/cloudflare-frameworks/sveltekit` — SvelteKit integration implementing
 * framework-core's `Framework` service, with the deploy target passed as a
 * value (Cloudflare Workers by default, via the
 * `@alchemy.run/cloudflare-frameworks/sveltekit/cloudflare` subpath module).
 *
...
/**
 * The e2e-harness factory contract (`framework: "@alchemy.run/cloudflare-frameworks/sveltekit"`
 * in `e2e.config.ts`).
 */
const factory = (
  options: HarnessOptions,
