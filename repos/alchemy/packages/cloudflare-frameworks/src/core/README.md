# @alchemy.run/cloudflare-frameworks/core
...
- **Framework packages** (`@alchemy.run/cloudflare-frameworks/waku`, `.../astro`,
...
- **Per-framework target modules** own the halves that are neither generic
  ...
  subpath of its framework package (e.g. `@alchemy.run/cloudflare-frameworks/waku/cloudflare`
  exporting the `WakuCloudflareTarget`). A future AWS target is a new subpath
  (`@alchemy.run/cloudflare-frameworks/waku/aws`) implementing the same seams — no change to
...
1. Its options gain `target?: <Framework>TargetInput` (a
   ...
   cloudflare subpath specifier (e.g. `"@alchemy.run/cloudflare-frameworks/waku/cloudflare"`).
...
- **Waku** — `WakuTarget` extends `DeployTarget` with the adapter-path hook
  (today: the wrangler-free fork at `@alchemy.run/cloudflare-frameworks/waku/adapter`, selected
