...
const assertSupportedOptions = (options: DistilledCloudflareOptions): void => {
  ...
  if (offending.length > 0) {
    throw new Error(
      "@alchemy.run/cloudflare-frameworks/astro: unsupported @astrojs/cloudflare option(s):\n" +
        offending
    ...
    hooks: {
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              ...
              cloudflarePlugins,
              {
                name: "@alchemy.run/cloudflare-frameworks/astro:cf-imports",
                enforce: "pre",
                resolveId: {
              ...
              {
                name: "@alchemy.run/cloudflare-frameworks/astro:environment",
              ...
              {
                enforce: "post",
                name: "@alchemy.run/cloudflare-frameworks/astro:cf-externals",
      ...
      "astro:config:done": ({
        ...
        if (config.adapter !== undefined && !_injectedAdapterMarker) {
          throw new Error(
            `@alchemy.run/cloudflare-frameworks/astro: the Astro config declares the adapter "${config.adapter.name}", ` +
        ...
        injectTypes({
          ...
          content:
            '/// <reference types="@alchemy.run/cloudflare-frameworks/astro/types.d.ts" />',
        });
        setAdapter({
          name: "@alchemy.run/cloudflare-frameworks/astro",
          adapterFeatures: {
