...
import { EchoReply } from 'hal-shared/echo';
 ...
 *
 * The "typed end to end" half is proven by `npm run typecheck` — the Worker
 * gets its `Sessions` client from the same declaration that provisions the
 * namespace, so a mismatch is a compile error rather than a runtime 500.
 *
 * This file proves the other half: that it actually runs. It stands up the
 * real stack and drives it over HTTP.
 *
 ...
 */
const enabled = process.env.HAL_E2E === '1';
...
const decodeReply = Schema.decodeUnknownOption(EchoReply);
...
test.skipIf(!enabled)(
  ...
  Effect.gen(function* () {
    ...
    const health = yield* Vitest.executeWhenReady(HttpClientRequest.get(`${baseUrl}/health`));
    ...
    const call = (session: string, text: string) =>
      Effect.gen(function* () {
        ...
        if (reply._tag !== 'Some') throw new Error('reply did not match EchoReply');
        return reply.value;
      });
...
test.skipIf(!enabled)(
  ...
  Effect.gen(function* () {
    ...
    const response = yield* client.execute(HttpClientRequest.get(`${deployed.url}/echo/alpha`));
