# @alchemy.run/cloudflare-frameworks/waku

Wrangler-free [Waku](https://waku.gg) integration. Implements
`@alchemy.run/cloudflare-frameworks/core`'s `Framework` service — programmatic
...
`@alchemy.run/cloudflare-frameworks/waku/cloudflare`.

## Architecture: framework half × deploy-target half

The package separates two concerns (see
`packages/cloudflare-frameworks/src/core/README.md` → "Architecture: frameworks × deploy
...
| `./source` (`src/waku/source.ts`)         | Alchemy `Cloudflare.Worker` source provider (`{ provider: "@alchemy.run/cloudflare-frameworks/waku/source" }`) wrapping the framework with the cloudflare target | `cloudflare-runtime` (types)       |
...
### The cloudflare target (`@alchemy.run/cloudflare-frameworks/waku/cloudflare`)
...
```ts
import wakuFramework, { make } from "@alchemy.run/cloudflare-frameworks/waku";

make({
  /** Deploy target: value | (config) => target | module specifier |
   *  harness carriage. Default: "@alchemy.run/cloudflare-frameworks/waku/cloudflare",
...
- a **target value** — full type safety over the config; build it yourself:
  ...
  ```ts
  import cloudflareTarget from "@alchemy.run/cloudflare-frameworks/waku/cloudflare";
...
export default Options.make({
  framework: "@alchemy.run/cloudflare-frameworks/waku",
  target: {
    cloudflare: {
      worker: {
