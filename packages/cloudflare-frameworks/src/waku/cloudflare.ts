/**
 * The Cloudflare Workers deploy target for `@alchemy.run/cloudflare-frameworks/waku` — the ONLY
 ...
 *   adapter (`@alchemy.run/cloudflare-frameworks/waku/adapter`): behavior-identical to upstream
    ...
    adapter: () =>
      Effect.try({
        try: () =>
          fileURLToPath(
            import.meta
              .resolve("@alchemy.run/cloudflare-frameworks/waku/adapter"),
          ),
        catch: (cause) =>
          new DeployTargetError({
            platform: "cloudflare",
            message:
              "Failed to resolve the @alchemy.run/cloudflare-frameworks/waku adapter module",
            cause,
          }),
      }),
    vitePlugins: (context) =>
