/**
 * `@alchemy.run/cloudflare-frameworks/nuxt` — Nuxt integration implementing framework-core's
 * `Framework` service, with the deploy target passed as a value (Cloudflare
 * Workers by default, via the `@alchemy.run/cloudflare-frameworks/nuxt/cloudflare` subpath
 * module).
 *
 ...
 * behind `@alchemy.run/cloudflare-frameworks/nuxt/cloudflare`.
 */
...
/**
 * The e2e-harness factory contract (`framework: "@alchemy.run/cloudflare-frameworks/nuxt"` in
 * `e2e.config.ts`).
 */
const factory = (
