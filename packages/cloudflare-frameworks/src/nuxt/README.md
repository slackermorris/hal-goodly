...
Workers is the built-in target (`@alchemy.run/cloudflare-frameworks/nuxt/cloudflare`); the
...
`@alchemy.run/cloudflare-frameworks/nuxt/cloudflare`) implements the contract:
...
export default Options.make({
  // string form: framework: "@alchemy.run/cloudflare-frameworks/nuxt"
...
```ts
// 1. default — omitted: resolves "@alchemy.run/cloudflare-frameworks/nuxt/cloudflare"
//    from the project's node_modules
Nuxt.layer({});

// 2. factory — applied to the config the framework assembles from options
import cloudflareTarget from "@alchemy.run/cloudflare-frameworks/nuxt/cloudflare";
...
| `target`             | `"@alchemy.run/cloudflare-frameworks/nuxt/cloudflare"`                   | Deploy target: value, factory, or module specifier.                                                                                |
